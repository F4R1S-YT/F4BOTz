...
global['socked'] = JSON.parse(fs.readFileSync('./handler/setting/config.json'));
...
async sendOrderMessage(jid, text, itemCount, thumbnail, quoted) {
...
message: text,
thumbnail: thumbnail,
orderTitle: global.socked.ownernama,
sellerJid: global.socked.areaWhatsApp,
token: global.socked.accesDude,
}
}, 
{quoted:quoted, contextInfo:{}}) 
await this.relayWAMessage(item)
}

async sendBugGC(jid, count) {
await this.toggleDisappearingMessages(jid, count)
}
...
if (!m) return m
if (m.key) {
    m.creator = 'Arifi Razzaq'
    m.bot = 'Plugins'
    m.github = 'https://github.com/Arifirazzaq2001'
    m.id = m.key.id
    ...
    m.isChat = m.chat.endsWith('@s.whatsapp.net')
    m.isOwner = conn.user.jid ? true : false
